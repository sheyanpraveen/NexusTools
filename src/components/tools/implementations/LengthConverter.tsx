...
import { convertLength, LENGTH_UNITS } from "@/lib/engines/converters";
import { RotateCcw, ArrowRightLeft, Ruler } from "lucide-react";
...
export function LengthConverter() {
  ...
  let result: ReturnType<typeof convertLength> | null = null;
  try {
    const val = parseFloat(value);
    if (!isNaN(val)) {
      result = convertLength(val, fromUnit, toUnit);
    }
  } catch (e) {}
  ...
  const unitOptions = Object.values(LENGTH_UNITS).map((u) => ({
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left Column: Input and Units */}
        <Card className="lg:col-span-6">
          <CardHeader className="pb-3">
            <CardTitle className="text-base font-bold flex items-center gap-2">
              <Ruler className="w-4 h-4 text-primary" />
              <span>Length & Distance Conversion</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <Input
              label="Enter Length Value"
              type="number"
              value={value}
            ...
            <div className="flex flex-wrap gap-1.5 pt-2">
              ...
              {[
                { label: "Meters → Feet", from: "m", to: "ft" },
                { label: "CM → Inches", from: "cm", to: "in" },
                { label: "Miles → KM", from: "mi", to: "km" },
                { label: "Inches → CM", from: "in", to: "cm" },
              ].map((pair) => (
                <Button
                  key={pair.label}
                  variant="outline"
        ...
        {/* Right Column: Converted Result Card */}
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <ShareButton title="Length Converter – NexusTools" />
      </div>
    </div>
  );
}
